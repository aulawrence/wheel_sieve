...
def get_difference_seq(coeff, x0, d):
    """Given coefficients [a0, a1, ..., an], x0 and d, compute [d0, d1, ..., dn] where di is the
    first term of the i-th order difference sequence of {f(x0), f(x0+d), f(x0+2d), ...}.
    With this sequence, we can compute {f(x0), f(x0+d), f(x0+2d), ...} sequentially using n-1
    ...
    Returns:
        list(int): [d0, d1, ..., dn] where di is the first term of the i-th order difference
        sequence of {f(x0), f(x0+d), f(x0+2d), ...}.
    """
    n = len(coeff)
...
def step_difference_seq_exn(pt_list, curve):
    """Compute 1 step forward on the points generated by a difference sequence. Modifies pt_list
    in place.
...
def ecm(n, rounds, b1, b2):
    ...
    for round_i in range(rounds):
        ...
        try:
            ...
            while c < c2 - c1:
                # assert cq_list[0] == wst.mul_pt_exn(
                #     q, wst_curve, apply_polynomial(polynomial, (c + c1) * wheel)
                # )
